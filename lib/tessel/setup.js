...
function checkAuthFile (tessel, authFile, callback) {
  tessel.connection.exec(commands.ensureFile(authFile), function (err, streams) {
...
function checkIfKey (tessel, authFile, pubKey, callback) {
  var tempData = '';
  tessel.connection.exec(commands.readFile(authFile), function (err, streams) {
    err && console.log(err);
    ...
    streams.stdout.once('end', function () {
      if(tempData.indexOf(pubKey) > -1) {
        console.log('Tessel is alreay authenticated with this computer.');
        closeAndExit(tessel);
      } else {
        callback && callback();
      }
...
function copyKey (tessel, authFile, pubKey) {
  tessel.connection.exec(commands.appendStdinToFile(authFile), function (err, streams) {
    err && console.log(err);
